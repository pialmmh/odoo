// ContactDetail — Fluent UI v9 clone of pages/crm/ContactDetail.jsx.
...
import {
  ...
  Person24Regular, Person20Regular, Mail20Regular, Phone20Regular,
  Building20Regular, Location20Regular,
...
import { getContact, deleteContact } from '../../services/crm-via-odoo';
...
export default function ContactDetail() {
  ...
  if (error || !row) {
    return <MessageBar intent="error">{error || 'Contact not found'}</MessageBar>;
  }
  ...
  const mainPanels = (
    <>
      <Panel title="Overview">
        <FieldGrid>
          <Field label="Name"  icon={<Person20Regular   />} value={fullName} />
          ...
          <Field label="Mobile" icon={<Phone20Regular />} value={row.mobileNumber
            ? <FluentLink href={`tel:${row.mobileNumber}`}>{row.mobileNumber}</FluentLink> : null} />
  ...
  return (
    <>
      <EntityDetailLayout
        ...
        title={fullName}
        subtitle={row.accountName || row.title || null}
        actionBar={actionBar}
