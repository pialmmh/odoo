...
import {
  ...
  Grid, FormControl, InputLabel, Select, MenuItem, Box,
  Chip, Autocomplete, Table, TableHead, TableBody, TableRow, TableCell,
  Typography,
} from '@mui/material';

// ── Shared section wrapper — matches PropSection style from detail pane ──
function EditSection({ title, children }) {
  return (
    <Box sx={{ mb: 1.5, border: '1px solid #e0e0e0', borderRadius: 1, overflow: 'hidden' }}>
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent sx={{ pt: '16px !important' }}>
        <EditSection title="General">
          ...
          <Grid size={{ xs: 6 }}><TextField fullWidth size="small" label="Code" required value={form.code || ''} onChange={e => setForm({ ...form, code: e.target.value })} /></Grid>
          <Grid size={{ xs: 6 }}>
          ...
          <Grid size={{ xs: 6 }}>
            <FormControl fullWidth size="small"><InputLabel>Zone Type</InputLabel>
              <Select label="Zone Type" value={form.zone_type || 'standard'} onChange={e => setForm({ ...form, zone_type: e.target.value })}>
        ...
        <EditSection title="Status">
          ...
          <Grid size={{ xs: 6 }}>
            <FormControl fullWidth size="small"><InputLabel>Default Zone</InputLabel>
              <Select label="Default Zone" value={form.is_default ? 'yes' : 'no'} onChange={e => setForm({ ...form, is_default: e.target.value === 'yes' })}>
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent sx={{ pt: '16px !important' }}>
        ...
        <EditSection title="Location">
          <Grid size={{ xs: 12 }}><TextField fullWidth size="small" label="Address" value={form.location_address || ''} onChange={e => setForm({ ...form, location_address: e.target.value })} /></Grid>
          <Grid size={{ xs: 6 }}><TextField fullWidth size="small" label="Latitude" type="number" value={form.latitude || 0} onChange={e => setForm({ ...form, latitude: +e.target.value })} /></Grid>
          <Grid size={{ xs: 6 }}><TextField fullWidth size="small" label="Longitude" type="number" value={form.longitude || 0} onChange={e => setForm({ ...form, longitude: +e.target.value })} /></Grid>
          <Grid size={{ xs: 6 }}>
            <FormControl fullWidth size="small"><InputLabel>DR Site</InputLabel>
              <Select label="DR Site" value={form.is_dr_site ? 'yes' : 'no'} onChange={e => setForm({ ...form, is_dr_site: e.target.value === 'yes' })}>
                <MenuItem value="no">No</MenuItem><MenuItem value="yes">Yes</MenuItem>
              </Select></FormControl>
          </Grid>
        </EditSection>
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent sx={{ pt: '16px !important' }}>
        ...
        {record && ipAddresses.length > 0 && (
          <EditSection title="IP Addresses">
            <Grid size={{ xs: 12 }}>
              <Table size="small">
                <TableHead><TableRow>
                ...
                <TableBody>
                  {ipAddresses.map(ip => (
                    <TableRow key={ip.id}>
                      <TableCell><Typography fontSize={12} fontFamily="monospace">{ip.ip_address}</Typography></TableCell>
                      <TableCell>{ip.ip_type}</TableCell>
...
export function ComputeModal({ open, onClose, onSave, record }) {
  ...
  useEffect(() => {
    if (open) {
      import('../../services/infra').then(m => m.getOSVersions([['is_active', '=', true]])).then(setOsVersions).catch(() => {});
    }
  }, [open]);
  ...
  useEffect(() => {
    if (record) {
      setForm({
        ...
        os_version_id: record.os_version_id?.[0] || null,
        management_ip: record.management_ip || '', status: record.status,
      });
    } else {
      setForm({ name: '', hostname: '', node_type: 'dedicated_server', cpu_cores: 0, memory_gb: 0, disk_gb: 0, brand: '', model: '', serial_number: '', rack_location: '', os_type: 'linux', os_version_id: null, management_ip: '', status: 'active' });
    }
  }, [record, open]);

  const selectedOsVersion = osVersions.find(v => v.id === form.os_version_id) || null;

  const handleSave = async () => {
    if (!form.name) return;
    setSaving(true);
    try { await onSave({ ...form, os_version_id: form.os_version_id || false }); onClose(); } catch (e) { alert(e.message); } finally { setSaving(false); }
  };
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent sx={{ pt: '16px !important' }}>
        ...
        <EditSection title="Operating System">
          <Grid size={{ xs: 12 }}>
            <Autocomplete
              size="small"
              options={osVersions}
              groupBy={(opt) => opt.family_id?.[1] || 'Other'}
              getOptionLabel={(opt) => opt.display_name || ''}
              value={selectedOsVersion}
              onChange={(_, newVal) => setForm({ ...form, os_version_id: newVal?.id || null })}
              renderInput={(params) => <TextField {...params} label="OS Version" placeholder="Search OS..." />}
              renderOption={(props, opt) => (
                <li {...props} key={opt.id}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, width: '100%' }}>
                    <Typography fontSize={13} sx={{ flexGrow: 1 }}>{opt.display_name}</Typography>
                    {opt.lts && <Chip label="LTS" size="small" color="success" sx={{ height: 16, fontSize: 9 }} />}
                    {opt.arch && <Typography fontSize={10} color="text.secondary">{opt.arch}</Typography>}
                  </Box>
                </li>
              )}
        ...
        <EditSection title="Location">
          <Grid size={{ xs: 12 }}><TextField fullWidth size="small" label="Rack Location" value={form.rack_location || ''} onChange={e => setForm({ ...form, rack_location: e.target.value })} /></Grid>
        </EditSection>
      </DialogContent>
      <DialogActions><Button onClick={onClose}>Cancel</Button><Button variant="contained" onClick={handleSave} disabled={saving}>{saving ? 'Saving...' : 'Save'}</Button></DialogActions>
    </Dialog>
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent sx={{ pt: '16px !important' }}>
        <EditSection title="Container">
          ...
          <Grid size={{ xs: 12 }}><TextField fullWidth size="small" label="Image" value={form.image || ''} onChange={e => setForm({ ...form, image: e.target.value })} /></Grid>
        </EditSection>
        <EditSection title="Resources">
