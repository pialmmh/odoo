...
import {
  listChannelCredentials, deleteChannelCredential, CHAT_CHANNELS,
} from '../../../../services/crm';
...
export default function ChannelCredentials() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await listChannelCredentials(params);
      setRows(res.list || []);
  ...
  const onDelete = async (r) => {
    ...
    try { await deleteChannelCredential(r.id); load(); }
  ...
  return (
    <Box sx={{ px: 3, py: 2.5 }}>
      <AdminPageHeader
        ...
        subtitle="Per-channel credentials (Facebook, WhatsApp, SMS, Email) used by the messaging bridge."
        actions={
          <>
            <Button variant="contained" startIcon={<AddIcon />} size="small"
              onClick={() => navigate(`${base}/crm/admin/channelCredentials/new`)}>
              Create
            </Button>
                ...
                <TableBody>
                  {rows.map((r) => (
                    <TableRow key={r.id} hover sx={{ cursor: 'pointer' }}
                      onClick={() => navigate(`${base}/crm/admin/channelCredentials/${r.id}/edit`)}>
                  ...
                  {rows.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={5} align="center" sx={{ py: 4, color: 'text.secondary' }}>
                        No channel credentials configured.
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
