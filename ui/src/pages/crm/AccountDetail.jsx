...
import {
  ...
  Alert, Link as MuiLink, Breadcrumbs, Chip,
} from '@mui/material';
import {
...
export default function AccountDetail() {
  ...
  const handleFollow = async () => {
    try {
      if (row.isFollowed) await unfollowEntity('Account', id);
      else                await followEntity('Account', id);
      load();
  ...
  const handleDuplicate = async () => {
    ...
    try {
      const { id: _i, createdAt, modifiedAt, createdById, modifiedById, createdByName, modifiedByName,
              isFollowed, followersIds, followersNames, versionNumber, ...rest } = row;
      const created = await createAccount(rest);
  ...
  if (error || !row) {
    return (
      <Box>
        <Breadcrumbs sx={{ mb: 2 }}><MuiLink component={RouterLink} to=".." underline="hover">Accounts</MuiLink><Typography color="text.primary">Not found</Typography></Breadcrumbs>
        <Alert severity="error">{error || 'Account not found'}</Alert>
      </Box>
    );
  }

  const billing  = [row.billingAddressStreet,  row.billingAddressCity,  row.billingAddressState,  row.billingAddressCountry,  row.billingAddressPostalCode ].filter(Boolean).join(', ');
  const shipping = [row.shippingAddressStreet, row.shippingAddressCity, row.shippingAddressState, row.shippingAddressCountry, row.shippingAddressPostalCode].filter(Boolean).join(', ');
    ...
    <Box>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'minmax(0, 1fr) 320px' }, gap: 2 }}>
        <Box sx={{ minWidth: 0 }}>
          <Panel title="Overview">
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2 }}>
              <FieldRow kind="account" label="Name" value={row.name} />
              <FieldRow kind="url"     label="Website" value={row.website} />
              <FieldRow kind="email"   label="Email"   value={row.emailAddress} />
              <FieldRow kind="tel"     label="Phone"   value={row.phoneNumber} />
              <FieldRow kind="address" label="Billing Address"  value={billing} multiline />
              <FieldRow kind="address" label="Shipping Address" value={shipping} multiline />
            </Box>
          </Panel>

          <Panel title="Details">
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2 }}>
              <FieldRow label="Type" value={row.type ? <Chip label={row.type} size="small" variant="outlined" /> : null} />
          ...
          <StreamPanel entityType="Account" id={row.id} />
        </Box>
        ...
        <Box>
          ...
          <ActivitiesPanel entityType="Account" id={row.id} parentName={row.name}
            refreshKey={sideRefresh} onRefresh={() => setSideRefresh(n => n + 1)} />
          <HistoryPanel entityType="Account" id={row.id} refreshKey={sideRefresh} />
          <TasksPanel   entityType="Account" id={row.id} parentName={row.name}
            refreshKey={sideRefresh} onRefresh={() => setSideRefresh(n => n + 1)} />
        </Box>
      </Box>
    </Box>
