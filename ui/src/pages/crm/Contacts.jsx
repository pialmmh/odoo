...
import { listContacts, deleteContact } from '../../services/crm';

// Columns mirror layouts/Contact/list.json
const COLUMNS = [
  { key: 'name',         label: 'Name' },
...
export default function Contacts() {
  ...
  const handleDelete = async (row, e) => {
    ...
    try { await deleteContact(row.id); load(); }
                  ...
                  {rows.map(r => (
                    <TableRow key={r.id} hover>
                      <TableCell>
                        ...
                        {r.title && <Typography variant="caption" color="text.secondary" component="div">{r.title}</Typography>}
                      </TableCell>
                      <TableCell><Typography variant="body2">{r.accountName || '—'}</Typography></TableCell>
                      <TableCell><Typography variant="body2" sx={{ fontSize: 12 }}>{r.emailAddress || '—'}</Typography></TableCell>
                      <TableCell><Typography variant="body2" sx={{ fontSize: 12 }}>{r.phoneNumber || '—'}</Typography></TableCell>
