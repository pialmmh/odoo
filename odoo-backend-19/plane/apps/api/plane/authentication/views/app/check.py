...
class EmailCheckEndpoint(APIView):
    ...
    def post(self, request):
        ...
        # When Odoo-backed auth is on, the field carries res.users.login
        # (which can be a username like 'admin'). Skip the email-format check
        # and always route to the CREDENTIAL (password) flow so the user can
        # enter their Odoo password — Plane has no local users to lookup,
        # they're get-or-created from Odoo on successful auth.
        odoo_auth_enabled = os.environ.get("ODOO_AUTH_ENABLED", "0") == "1"

        if not odoo_auth_enabled:
            # Validate email
            try:
