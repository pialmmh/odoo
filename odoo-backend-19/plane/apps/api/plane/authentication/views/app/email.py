...
class SignInAuthEndpoint(View):
    def post(self, request):
        ...
        # When Odoo-backed auth is on, the field carries res.users.login (e.g. "admin"),
        # and the local Plane user row may not exist yet — JIT-created from Odoo's
        # response. Skip the email-format check and the User-exists pre-check so the
        # EmailProvider's Odoo path can run; check.py already does the same.
