...
def unarchive_archive_page_and_descendants(page_id, archived_at):
    ...
    WITH RECURSIVE descendants AS (
        SELECT id FROM plane_pages WHERE id = %s
        UNION ALL
        SELECT plane_pages.id FROM plane_pages, descendants WHERE plane_pages.parent_id = descendants.id
    )
    UPDATE plane_pages SET archived_at = %s WHERE id IN (SELECT id FROM descendants);
    """
