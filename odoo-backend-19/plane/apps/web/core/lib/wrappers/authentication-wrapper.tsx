...
export const AuthenticationWrapper = observer(function AuthenticationWrapper(props: TAuthenticationWrapper) {
  ...
  // Wait for *all* sources before any routing decision. The original guard only
  // gated on `!currentUser?.id`, which left a window where currentUser was set
  // but currentUserProfile was still in-flight — the AUTHENTICATED branch then
  // treated "profile undefined" as "not onboarded" and pushed to /onboarding/,
  // briefly showing the wizard even when is_onboarded=true on the server.
  if (
    isUserSWRLoading ||
    isUserLoading ||
    workspacesLoader ||
