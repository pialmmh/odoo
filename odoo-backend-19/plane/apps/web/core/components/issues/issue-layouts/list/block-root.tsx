...
export const IssueBlockRoot = observer(function IssueBlockRoot(props: Props) {
  ...
  // states — WBS view: parents start expanded so children render indented like MS Project
  ...
  // WBS view: eagerly fetch sub-issues for any parent so the recursive renderer
  // can draw them indented without waiting for a chevron click. Skipped for epics
  // (they render via a different path) and only fires when the store hasn't loaded yet.
  useEffect(() => {
    const workspaceSlug = routerWorkspaceSlug?.toString();
    if (!workspaceSlug || !issue?.project_id || subIssuesCount === 0 || isEpic) return;
    if (subIssuesStore.subIssuesByIssueId(issueId) !== undefined) return;
    subIssuesStore.fetchSubIssues(workspaceSlug, issue.project_id, issueId);
  }, [routerWorkspaceSlug, issue?.project_id, issueId, subIssuesCount, isEpic, subIssuesStore]);
