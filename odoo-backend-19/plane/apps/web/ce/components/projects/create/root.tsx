    ...
    if (coverImage) {
      ...
      if (imageType === "local_static") {
        try {
          ...
          // Upload failed (e.g. MinIO/S3 misconfigured). Fall back to the static URL as the
          // legacy cover_image field so project creation still succeeds with a default image.
          console.warn("Cover image upload failed; falling back to static URL.", error);
          setToast({
            type: TOAST_TYPE.WARNING,
            title: t("toast.warning"),
            message: "Could not upload cover image — using default image instead.",
          });
