...
export const WORKSPACE_SIDEBAR_STATIC_NAVIGATION_ITEMS: Record<string, IWorkspaceSidebarNavigationItem> = {
  ...
  // CRM — talks to Odoo crm.lead via the Spring Boot gateway. Custom Telcobright add-on
  // to Plane's CE sidebar; the page lives at app/(all)/[workspaceSlug]/(projects)/crm/leads/.
  "crm-leads": {
    key: "crm_leads",
    labelTranslationKey: "CRM — Leads",
    href: `/crm/leads`,
    access: [EUserWorkspaceRoles.ADMIN, EUserWorkspaceRoles.MEMBER, EUserWorkspaceRoles.GUEST],
    highlight: (pathname: string, url: string) => pathname.includes(url),
  },
  // Orchestrix — placeholder root for the gradual port of orchestrix-ui infra
  // management into Plane's sidebar. Subsections (containers, networking, ...)
  // will be added incrementally; this entry is the doorway.
  orchestrix: {
    key: "orchestrix",
    labelTranslationKey: "Orchestrix",
    href: `/orchestrix`,
    access: [EUserWorkspaceRoles.ADMIN, EUserWorkspaceRoles.MEMBER, EUserWorkspaceRoles.GUEST],
    highlight: (pathname: string, url: string) => pathname.includes(url),
  },
...
export const WORKSPACE_SIDEBAR_STATIC_PINNED_NAVIGATION_ITEMS_LINKS: IWorkspaceSidebarNavigationItem[] = [
  ...
  WORKSPACE_SIDEBAR_STATIC_NAVIGATION_ITEMS["crm-leads"],
  WORKSPACE_SIDEBAR_STATIC_NAVIGATION_ITEMS["orchestrix"],
];
